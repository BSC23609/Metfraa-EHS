...
// `approval` is optional: { reviewerName, reviewerEmail, reviewedAt, status }
//                         — included for approved submissions
async function generatePdfReport(form, submission, photoBuffers, approval = null) {
...
function drawSubmissionMeta(doc, submission, approval) {
  doc.fillColor(COLOR_BLACK).font('Helvetica-Bold').fontSize(9);
  const startY = doc.y;
  const isApproved = approval && approval.status === 'Approved';

  // Reserve space for the APPROVED badge if present
  const badgeW = isApproved ? 130 : 0;
  const badgeGap = isApproved ? 16 : 0;
  const usableWidth = doc.page.width - 100 - badgeW - badgeGap;
  ...
  // If approved, draw a green "APPROVED" badge in the top-right corner
  if (isApproved) {
    const badgeH = 30;
    const badgeX = doc.page.width - 50 - badgeW;
    const badgeY = startY;
    doc.rect(badgeX, badgeY, badgeW, badgeH).fill(COLOR_GREEN);
    doc.fillColor('#FFFFFF').font('Helvetica-Bold').fontSize(13)
       .text('APPROVED', badgeX, badgeY + 9, { width: badgeW, align: 'center', lineBreak: false });
  }
}
...
// Approval block — shown at bottom of report when submission has been approved
function drawApprovalBlock(doc, approval) {
  ensureSpace(doc, 80);
  doc.moveDown(1);
  const x = 50;
  ...
  doc.rect(x, y, w, 70).fill('#F1FAF3');
  doc.rect(x, y, 4, 70).fill(COLOR_GREEN); // green left bar
  ...
  doc.text(`Reviewed at: ${reviewedAt}`,
           x + 16, y + 46, { width: w - 32, lineBreak: false });
