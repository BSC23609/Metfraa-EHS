...
async function loadFormSubmissions(form) {
  ...
  const keyCandidates = ['Equipment No.', 'Project Name', 'Site Name',
                         'Employee / Worker Name', 'Meeting No.',
  ...
  ws.eachRow({ includeEmpty: false }, (row, rowNum) => {
    ...
    const cellVal = (idx) => {
      ...
      // Excel cells can have hyperlink objects, formulas, etc.
      const v = c.value;
    ...
    rows.push({
      ...
      pdfLink: cellVal(colPdfLink), // OneDrive web URL (admin only — others use proxy)
    });
  });
